...
export default function Nav({ dark = false }: { dark?: boolean }) {
  ...
  const bdr = dark ? '1px solid rgba(255,255,255,0.15)' : '1px solid #cacaca'

  return (
    <nav style={{ background: bg, borderBottom: bdr, position: 'sticky', top: 0, zIndex: 50 }}>
      ...
      <div className="hidden md:flex" style={{ ...PX, paddingTop: 16, paddingBottom: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 16, textDecoration: 'none', color: fg }}>
          <span style={{ width: 38, height: 38, color: fg, display: 'block' }}>{LOGO_SVG}</span>
          <span className="text-h3" style={{ letterSpacing: '-0.04em' }}>wannabe</span>
        </a>
        <div style={{ display: 'flex', gap: 24 }}>
          {LINKS.map(l => (
            <a key={l} href="#" className="text-body" style={{ textDecoration: 'none', color: fg, letterSpacing: '0.01em' }}>{l}</a>
          ))}
        </div>
      </div>
      ...
      <div className="flex md:hidden" style={{ paddingLeft: 24, paddingRight: 24, paddingTop: 16, paddingBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 16, textDecoration: 'none', color: fg }}>
          <span style={{ width: 30, height: 30, color: fg, display: 'block' }}>{LOGO_SVG}</span>
          <span className="text-h3" style={{ letterSpacing: '-0.04em' }}>wannabe</span>
        </a>
        <button className="btn-menu" style={{ color: fg }} onClick={() => setOpen(o => !o)}>
          {open ? '×меню' : '+меню'}
        </button>
      </div>
      ...
      {/* ── Mobile overlay ─────────────────────────── */}
      {open && (
        <div style={{ position: 'fixed', inset: 0, background: bg, zIndex: 40, padding: '80px 24px 24px', display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
          {LINKS.map(l => (
            <a key={l} href="#" onClick={() => setOpen(false)}
              className="text-h4" style={{ textDecoration: 'none', color: fg }}>{l}</a>
          ))}
